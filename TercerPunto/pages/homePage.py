...
class HomePage:
    def __init__(self, driver):
        ...
        self.pos_verification_element = (By.XPATH,"//button[@id='pointOfSaleSelectorId']/span[2]/text()")
    ...
    def select_language(self, language):
        ...
        lang_option_xpath = self.language_option.format(language)  # Usamos format() en la cadena para que obtenga el valor del idioma que esperamos seleccionar
    ...
    def select_pos(self, pos):
        ...
        self.driver.find_element(*self.pos_dropdown).click()

        pos_option_xpath = self.pos_option.format(pos)  # Usamos format() en la cadena para que obtenga el valor del pais que esperamos seleccionar
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH, pos_option_xpath)))
        self.driver.find_element(By.XPATH, pos_option_xpath).click()
        self.driver.find_element(*self.pos_confirm_selection).click()
    ...
    def verify_pos_change(self, expected_text):
        # Espera explícita hasta que el mensaje de bienvenida esté visible
        actual_text = WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located(self.pos_verification_element)).text
        return actual_text == expected_text
